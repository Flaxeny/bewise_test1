...
async def call_for_quiz_items(
    ...
    items: list[QuizItem | None] = await crud.select_all(db=db)
    background_tasks.add_task(crud.insert_quiz_items, count.count, db)
...
async def get_quiz_items(
    ...
    items = await crud.select_with_constraints(db=db, limit=limit, order_by=order_by, order=order)
    if not items:
...
async def get_items_count(db: AsyncSession = Depends(get_db)):
    count: int = await crud.get_items_count(db=db)
